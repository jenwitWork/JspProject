...
public interface DocumentRepository extends CrudRepository<Document, String> {
	...
	@Query(value = "select * from document where ((doc_no like %?1% and branch_id in (?2) and serie_title like %?4% and cm_name like %?5% and pb_name like %?6% and case_name_th like %?7% and case_name_en like %?8%) or (status like 'approved')) and (status like %?3%) order by doc_no desc", nativeQuery = true)
